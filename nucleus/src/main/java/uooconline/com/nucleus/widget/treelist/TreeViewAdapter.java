package uooconline.com.nucleus.widget.treelist;
...
/**
 * update by ricky 2017/10/1
 * Created by tlh on 2016/10/1 :) 分组列表：https://github.com/TellH/RecyclerTreeView
 */
public class TreeViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private boolean autoExpand;//自动赋予item点击收缩事件

    public TreeViewAdapter(List<? extends TreeViewBinder> viewBinders) {
        this(null, viewBinders, true);
    }

    public TreeViewAdapter(List<TreeNode> nodes, List<? extends TreeViewBinder> viewBinders, Boolean autoClickExpand) {
        displayNodes = new ArrayList<>();
        if (nodes != null)
            findDisplayNodes(nodes);
        this.viewBinders = viewBinders;
        this.autoExpand = autoClickExpand;
    }
        ...
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (onTreeNodeListener != null && onTreeNodeListener.onClick(selectedNode, holder))
                    return;
                if (autoExpand)
                    toggleNode(selectedNode, holder);
            }
        });
        for (TreeViewBinder viewBinder : viewBinders) {
            if (viewBinder.getLayoutId() == displayNodes.get(position).getContent().getLayoutId()) {
                ...
                viewBinder.onPaddingHeight(holder, displayNodes.get(position).getHeight());
            }
        }
    }

    public void toggleNode(TreeNode selectedNode, RecyclerView.ViewHolder holder) {
        ...
        if (!isExpand) {
            ...
            if (onTreeNodeListener != null)
                onTreeNodeListener.onToggleAfter(selectedNode, true, holder);
        } else {
            notifyItemRangeRemoved(positionStart, removeChildNodes(selectedNode, true));
            if (onTreeNodeListener != null)
                onTreeNodeListener.onToggleAfter(selectedNode, false, holder);
        }
    }
    ...
    public interface OnTreeNodeListener {
        ...
        void onToggleAfter(TreeNode selectedNode, boolean isInsert, RecyclerView.ViewHolder holder);
    }
